...
CREATE TABLE research_sessions (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  CONSTRAINT valid_phase CHECK (current_phase IN ('scout', 'analyst_triage', 'analyst', 'verifier', 'hypothesis', 'brancher', 'critic', 'writer', 'completed', 'waiting_approval')),
...
COMMENT ON COLUMN papers.embedding IS 'pgvector embedding (768 dims, Gemini text-embedding-004) for semantic similarity search';
...
CREATE TABLE approval_gates (
  ...
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  CONSTRAINT valid_gate_phase CHECK (phase IN ('scout', 'analyst_triage', 'analyst', 'verifier', 'hypothesis', 'brancher', 'critic', 'writer', 'budget_exceeded')),
...
-- ============================================================================
-- EVENTS TABLE - Real-time event stream for Supabase Realtime → UI
-- ============================================================================
...
CREATE TABLE events (
  ...
  CONSTRAINT valid_event_type CHECK (event_type IN (
    'phase_started', 'agent_progress', 'task_completed',
    'phase_completed', 'approval_required', 'error',
    'budget_warning', 'budget_exceeded', 'session_completed', 'rule_gate_updated'
  ))
);

COMMENT ON TABLE events IS 'Real-time event stream: n8n inserts rows, Supabase Realtime pushes to UI. Persistent for session history.';
COMMENT ON COLUMN events.event_type IS 'Event type matching UI handler registry';
COMMENT ON COLUMN events.payload IS 'Event-specific data: phase, message, gate_id, budget amounts, etc.';
...
-- ============================================================================
-- CLAIMS EMBEDDING - pgvector column for semantic claim search
-- ============================================================================

-- Note: claims.embedding added inline would require redefining the table.
-- Instead, add via ALTER TABLE (safe for migrations):
ALTER TABLE claims ADD COLUMN embedding vector(768);
CREATE INDEX idx_claims_embedding ON claims USING hnsw (embedding vector_cosine_ops);
COMMENT ON COLUMN claims.embedding IS 'pgvector embedding (768 dims, Gemini text-embedding-004) for semantic claim similarity';

-- ============================================================================
-- BUDGET TRACKING TRIGGER - Auto-update budget_spent on agent_runs INSERT
-- ============================================================================
...
COMMENT ON FUNCTION update_budget_spent IS 'Auto-increments research_sessions.budget_spent when agent_runs records cost';
