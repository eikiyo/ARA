...
|-----------|-----------|---------|
| **RAG (Vector DB)** | Qdrant (Docker container) | Semantic search across ingested papers |
| **Metadata DB** | PostgreSQL | Task queue, papers, claims, hypotheses, branches, audit log |
| **Workflow Engine** | n8n (Docker, queue mode) | Agent orchestration, parallel execution, feedback loops |
| **Queue Backend** | Redis | n8n queue mode for concurrent worker execution |
| **PDF Parser** | GROBID (Docker container) | Academic PDF → structured sections (title, abstract, methods, results, etc.) |
| **Embeddings** | Gemini text-embedding-004 (free tier, 768 dims) | Paper/claim vectorization. 1500 req/min free. |
| **LLM** | Claude API (Sonnet for agents, Opus for Critic/Hypothesis) | Agent reasoning |
| **Web UI** | Next.js (Docker container) | Live approval dashboard, session config, progress tracking |
| **WebSocket Server** | Node.js (lightweight, Docker) | Real-time push from n8n → UI via webhook callbacks |
| **Hosting** | Railway | All services as Docker containers with private networking |
...
| Web UI | JavaScript/React (Next.js) | Approval dashboard, session config |
| WebSocket server | JavaScript (Node.js) | ~50 lines, relays n8n webhooks to UI |
| Database | SQL | Schema + optimized queries |
| GROBID, Qdrant, Redis | Java/Rust/C (containers) | No code to maintain — pre-built images |
...
### 3.5 Optimistic Locking for Concurrent Agents
...
3. WebSocket server pushes gate data to UI
...
```
[n8n workflow] ──webhook POST──→ [WebSocket Server] ──ws push──→ [Next.js UI]
                                       ↑
[User action] ──HTTP POST──→ [Next.js API] ──webhook POST──→ [n8n webhook trigger]
```

- n8n posts phase updates, completions, and errors to the WS server via HTTP webhook
- WS server pushes to connected UI clients
- User actions (approve, reject, edit, revert) go through Next.js API → n8n webhook
- Single WebSocket connection per session (V1 = one session at a time)

### 4.9 WebSocket Message Protocol

All messages are JSON with a `type` field. n8n → WS Server → UI:
...
UI → Next.js API → n8n (via webhook POST):
...
- All session data persists **forever** in Postgres + Qdrant + file storage
...
Qdrant runs as a Docker container on Railway with persistent volume.

**n8n integration:** n8n has a native Qdrant Vector Store node. Use it for:
- Insert Documents mode (Scout stores papers)
- Retrieve as AI Agent Tool mode (all agents query)

### 8.2 Collections

| Collection | Vector Size | Distance | Payload Fields |
|-----------|-------------|----------|---------------|
| `paper_chunks` | 768 (Gemini) | Cosine | paper_id, session_id, title, section_type, chunk_index, source, year |
| `claims` | 768 (Gemini) | Cosine | claim_id, session_id, claim_text, verification_status |
...
### 8.4 Full Text Ingestion Pipeline

Papers are ingested with **full text preferred, abstract as fallback**:

```
[Scout finds paper]
    ↓
[Check: full text available?]
    ↓ YES (arXiv PDF, PubMed Central, CORE, Unpaywall)
[Download PDF]
    ↓
[GROBID: parse PDF → structured sections]
    ↓
[Chunk by section (see 8.5)]
    ↓
[Embed each chunk → Qdrant]

    ↓ NO (paywalled, no OA version)
[Use abstract only]
    ↓
[Embed abstract as single chunk → Qdrant]
```

**Unpaywall API** (`api.unpaywall.org`) is used to find free legal full-text versions of paywalled papers before falling back to abstract-only.
...
|---------|-------|---------|-------------------|
| n8n | `n8nio/n8n` | Workflow engine (queue mode) | Yes (workflows, credentials) |
| PostgreSQL | `postgres:16` | ARA metadata DB + n8n backend | Yes |
| Qdrant | `qdrant/qdrant` | Vector DB for RAG | Yes |
| Redis | `redis:7` | n8n queue mode backend | No (ephemeral) |
| GROBID | `lfoppiano/grobid:0.8.0` | Academic PDF parsing | No |
| Next.js UI | Custom Dockerfile | Approval dashboard + session config | No |
| WebSocket Server | Custom Dockerfile | Real-time n8n → UI push | No |
...
| AI Agent (Tools Agent) | Analyst, Hypothesis, Brancher, Critic, Writer | LLM reasoning with tools |
| HTTP Request | Scout, Verifier, GROBID calls, embedding API | External API calls |
| Qdrant Vector Store | All agents | RAG insert/retrieve |
...
V1: 4-digit PIN gate on the Next.js UI. No user accounts, no OAuth. PIN is set as an environment variable on Railway.

### 10.5 Concurrency Model

n8n runs in queue mode with `N8N_WORKER_CONCURRENCY=10` (10 concurrent workflow executions).
...
**Total peak concurrency:** ~10 workflows (during Scout or Verifier phases). Fits within `N8N_WORKER_CONCURRENCY=10`.
...
1. Docker Compose: n8n + Postgres + Qdrant + Redis + GROBID
2. Deploy to Railway, verify all services communicate
3. Run database migrations (ara_schema.sql + rule_gate table)
4. Build Manager workflow (Cron + task polling + approval gate state)
5. Build Scout sub-workflow (Semantic Scholar only, abstract + full text via GROBID)
6. Build Analyst sub-workflow (extract claims)
7. Build Verifier sub-workflow (retraction check only)
8. Test end-to-end: topic in → verified claims out (via n8n UI, no custom UI yet)
9. **Git checkpoint**

### Phase 2: Approval UI + Rule Gate
1. Build Next.js app: PIN gate, session config screen, live dashboard
2. Build WebSocket server for real-time updates
3. Implement approval gates (Approve/Reject/Edit/Revert) with n8n webhook integration
4. Implement Rule Gate: UI panel, Postgres table, prompt injection, post-validation
5. Connect revert behavior (one step back, keep old data + add new)
6. Test: full session with human approval at every step
7. **Git checkpoint**

### Phase 3: Intelligence Layer
1. Build Hypothesis Generator (20 ranked hypotheses with multi-dimensional scoring)
2. Build Brancher (all 4 types: lateral, methodological, analogical, convergent)
3. Build Critic with scoring dimensions + feedback loop
4. Add all 10 Scout sources (arXiv, PubMed, OpenAlex, CrossRef, CORE, DBLP, Europe PMC, BASE, Google Scholar)
5. Add Unpaywall for full-text discovery
6. Test: topic in → approved hypothesis out with UI
7. **Git checkpoint**

### Phase 4: Writer + Output
1. Build Writer sub-workflow (LaTeX output, all 6 paper types)
2. Writer → Critic → Writer feedback loop (max 3 iterations)
3. Build export pipeline: generate index.html, compile PDF via Pandoc, package ZIP
4. Build session history page (revisit past sessions, re-export)
5. Budget meter integration (real-time cost tracking in UI)
6. Error visibility in UI (retry, skip, abort)
7. Test: full end-to-end, show output to a researcher
8. **Git checkpoint**
...
|-----------|--------------------------------------|------|
| LLM (Claude Sonnet) | ~500K tokens (Scout, Analyst, Verifier, Writer) | ~$1.50 |
| LLM (Claude Opus) | ~100K tokens (Hypothesis, Critic) | ~$1.50 |
| LLM (Rule Gate post-validation) | ~50K tokens | ~$0.15 |
| Embeddings (Gemini) | Free tier (1500 req/min) | $0.00 |
| Academic APIs | All free tier | $0.00 |
| SerpAPI (Google Scholar) | ~$0.005/search | ~$0.05 |
| Infrastructure (Railway) | ~$20-40/mo shared across sessions | ~$0.50/session amortized |
| **Total per session** | | **~$3.70** |

**Presets:**
| Preset | Papers | Deep-read | Est. Cost |
|--------|--------|-----------|-----------|
| Quick scan | 20 | 20 (all) | ~$1.00 |
| Standard | 200 | 100 | ~$3.70 |
| Deep research | 500+ | 200 | ~$8.00+ |
...
| 5 | Hosting | Railway (Docker containers with private networking) |
...
| 8 | Authentication | 4-digit PIN for V1 (single user) |
| 9 | Paper ingestion | Full text preferred (GROBID parsing), abstract fallback |
| 10 | Embeddings | Gemini text-embedding-004 (free tier) |
...
| Qdrant crashes | Medium | Railway persistent volume, periodic snapshots |
...
| GROBID fails on malformed PDF | Medium | Fallback to abstract-only, LLM-based section splitting for raw text |
