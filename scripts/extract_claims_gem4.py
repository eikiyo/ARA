# Location: scripts/extract_claims_gem4.py
# Purpose: Gemini direct extractor — middle-right slice
...
_log = logging.getLogger("gem_extract_4")
...
def main():
    ...
    total = len(rows)
    # Take 4th sixth
    chunk = rows[3*total//6 : 4*total//6]
    ...
    _log.info("GEM-4: Extracting %d papers (sixth 4)", len(papers))
    ...
    for i, paper in enumerate(papers):
        ...
        try:
            ...
            if not claims: failed += 1
                ...
                if cc:
                    res = central_db.store_claims(cc, session_topic="prewarm_gem4")
    ...
    _log.info("GEM-4 DONE: %d claims from %d papers in %.1f min", total_claims, done, (time.time()-t0)/60)
