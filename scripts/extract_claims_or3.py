# Location: scripts/extract_claims_or3.py
# Purpose: Bulk claim extraction via OpenRouter Gemini Flash Lite — quarter 3
...
_log = logging.getLogger("or_extract_3")
...
def main():
    ...
    total = len(rows)
    quarter = rows[total // 2 : 3 * total // 4]
    papers = [dict(r) for r in quarter if not central_db.is_paper_fully_extracted(r["title"] or "")]
    if not papers: _log.info("All done"); return
    _log.info("OR-3: Extracting %d papers (quarter 3)", len(papers))
    ...
    for i, paper in enumerate(papers):
        ...
        try:
            ...
            if not claims: failed += 1
                ...
                if cc:
                    res = central_db.store_claims(cc, session_topic="prewarm_or3")
    ...
    _log.info("OR-3 DONE: %d claims from %d papers in %.1f min", total_claims, done, (time.time()-t0)/60)
