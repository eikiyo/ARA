# Location: scripts/extract_claims_nodoi.py
# Purpose: Parallel claim extraction for full-text papers WITHOUT DOI (15 agents via OpenRouter)
...
def progress_reporter(total: int):
    while True:
        ...
        _log.info(
            "PROGRESS [%.0fm] | Papers: %d/%d (%.0f/min) | Claims: %d | Failed: %d | ETA: %.0fm",
...
def main():
    ...
    _log.info("Full-text papers (no DOI, no claims): %d", len(papers))

    if not papers:
        _log.info("Nothing to extract!")
        return
    ...
    _log.info("=" * 60)
    _log.info(
        "DONE in %.1f min | Papers: %d/%d | Claims: %d | Failed: %d",
