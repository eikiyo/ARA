# Location: scripts/extract_claims_or4.py
# Purpose: Bulk claim extraction via OpenRouter Gemini Flash Lite — quarter 4
...
_log = logging.getLogger("or_extract_4")
...
def main():
    ...
    _log.info("OR-4: Extracting %d papers (quarter 4)", len(papers))
    ...
    for i, paper in enumerate(papers):
        ...
        try:
            ...
            if not claims: failed += 1
                ...
                if cc:
                    res = central_db.store_claims(cc, session_topic="prewarm_or4")
    ...
    _log.info("OR-4 DONE: %d claims from %d papers in %.1f min", total_claims, done, (time.time()-t0)/60)
