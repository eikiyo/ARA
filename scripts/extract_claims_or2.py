# Location: scripts/extract_claims_or2.py
# Purpose: Bulk claim extraction via OpenRouter Gemini Flash Lite — quarter 2 by paper_id
# Functions: Extract claims from second quarter of papers
...
_log = logging.getLogger("or_extract_2")
...
def main():
    ...
    _log.info("OR-2: Extracting %d papers (quarter 2 by paper_id)", len(papers))
    ...
    for i, paper in enumerate(papers):
        ...
        try:
            ...
            if not claims: failed += 1
                ...
                if cc:
                    res = central_db.store_claims(cc, session_topic="prewarm_or2")
    ...
    _log.info("OR-2 DONE: %d claims from %d papers in %.1f min", total_claims, done, (time.time()-t0)/60)
